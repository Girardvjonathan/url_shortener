...
class TestBenchmarkSeed:
    def test_seeds_all_postgres_urls_into_dynamo(self, client, mock_pool, mock_dynamo_session):
        mock_pool.fetch.return_value = [
            make_row(short_code="abc", full_url="https://example.com"),
            make_row(short_code="xyz", full_url="https://other.com"),
        ]
        table = setup_dynamo(mock_dynamo_session)

        resp = client.post("/api/benchmark/seed")
    ...
    def test_reports_dynamo_failures(self, client, mock_pool, mock_dynamo_session):
        mock_pool.fetch.return_value = [
            make_row(short_code="abc", full_url="https://example.com"),
            make_row(short_code="xyz", full_url="https://other.com"),
        ]
        setup_dynamo(mock_dynamo_session, put_raises=Exception("schema mismatch"))

        data = client.post("/api/benchmark/seed").json()

        assert data["seeded"] == 0
        assert data["failed"] == 2
        assert len(data["errors"]) == 2

    def test_empty_database_seeds_zero(self, client, mock_pool, mock_dynamo_session):
        mock_pool.fetch.return_value = []
        setup_dynamo(mock_dynamo_session)
